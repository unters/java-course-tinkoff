...
public class Task3Test {
    ...
    void personDatabaseImplementation_MultipleThreadsPerformOperationsOnASingleDatabase_RaceConditionDoesNotOccur() {
        ...
        Runnable addingRunnable = () -> {
            ...
            for (int i = 0; i < iterationsNumber; ++i) {
                personDatabase.add(generateNewPerson(randomGenerator));
            }
        };
        ...
        Runnable deletingRunnable = () -> {
            RandomGenerator randomGenerator = new Random();
            for (int i = 0; i < iterationsNumber; ++i) {
                personDatabase.delete(randomGenerator.nextInt(100));
            }
        };
    ...
    private static Person generateNewPerson(RandomGenerator randomGenerator) {
        int id = randomGenerator.nextInt(100);
        String name =
            (randomGenerator.nextInt(10) < 8) ? names[randomGenerator.nextInt(names.length)] : null;
        String address =
            (randomGenerator.nextInt(10) < 8) ? addresses[randomGenerator.nextInt(addresses.length)] : null;
        String phoneNumber =
            (randomGenerator.nextInt(10) < 8) ? phoneNumbers[randomGenerator.nextInt(phoneNumbers.length)] : null;
